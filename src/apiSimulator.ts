...
export const fetchProductReviews = (productId: number): Promise<{ id: number; productId: number; review: string; rating: number }[]> => {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (Math.random() < 0.8) {
                resolve([
                    { id: 1, productId: productId, review: "Worth every penny!", rating: 5 },
                    { id: 2, productId: productId, review: "Exceeds expectations", rating: 4 },
                    { id: 3, productId: productId, review: "Great product!", rating: 3 },
                    { id: 4, productId: productId, review: "Good value, not the best product", rating: 2 },
                    { id: 5, productId: productId, review: "Wish I would have went with a better option", rating: 1 }
                ]);
            } else {
